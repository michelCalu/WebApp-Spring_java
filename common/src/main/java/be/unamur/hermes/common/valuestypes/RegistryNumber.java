...
/**
 * this class creates a proper NRN respecting format: [YYMMDDABCDE] with YYMMDD
 * as birthdate and ABCDE a 5-digit number
 */
...
public class RegistryNumber {
    ...
    public static RegistryNumber create(String numberString) throws NRNNotValidException {

	if (numberString.length() != 11)
	    throw new NRNNotValidException("invalid NRN length");
	if (!numberString.matches("[0-9]+"))
	    throw new NRNNotValidException("invalid chars in NRN");
	...
	if (monthInt < 0 || monthInt > 12 || dayInt < 0 || dayInt > 31) {
	    throw new NRNNotValidException("invalid date format in NRN");
	}
