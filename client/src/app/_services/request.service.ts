...
export class RequestService {
    ...
    getCreationDate(requestId: number): Observable<Date> {
        return this.http
            .get(/*this.serverAddress + */ '/events?requestId=' + requestId + '&eventType=created')
            .map((events: RequestEvent[]) => {
                if (!events.length || !events[0].at) {
                    return null;
                }
                const dateArray = events[0].at;
                return new Date(dateArray[0], dateArray[1], dateArray[2], dateArray[3], dateArray[4], dateArray[5]);
            })
            .catch(err => {
                this.messageService.error(this.translateService.instant('request.service.getCreationDate'));
